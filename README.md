...
To provide for a better user experience, movies that are nominated by a user are preserved in local storage so that nominated movies that are added to the Nominations list are not lost when the page is refreshed. Likewise, when movies are removed from the Nominations list, they will remain removed from the list when the page is refreshed.
